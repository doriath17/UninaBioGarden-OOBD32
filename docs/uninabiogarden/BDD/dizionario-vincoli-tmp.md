...
- **Stato Finale**: se `stato = CONCLUSA`, allora:
  - **Congelamento Attributi**: ogni attributo della coltivazione non può essere più modificato.
  - **Annullamento a catena Attività Rimanenti**: lo `stato` di tutte le attività associate tali che `attivita.stato != COMPLETATA AND attivita.stato != ANNULLATA` deve essere impostato ad `ANNULLATA`.
  - **Fine Attività**: non è possibile pianificare ulteriori attività sulla coltivazione.

- **Annullamento a catena delle attività**: Quando una coltivazione entra nel suo stato finale **tutte** le sue attività sono **_congelate_**: lo stato di tutte le attività associate alla coltivazione, che non sono in uno stato terminale, diventa `ANNULLATO` e, per definizione dello stato finale di una attività, queste diventano immutabili. Inoltre alla coltivazione stessa non sarà più possibile associare nuove attività.
...
- **Stato Finale**: se `stato = CONCLUSO`, allora:
...
| **-**           | PIANIFICATO        |                                                                  |
| PIANIFICATO     | ATTIVO             |                                                                  |
| ATTIVO          | CONCLUSO           | solo se tutte le coltivazioni associate hanno `stato = CONCLUSA` |
| CONCLUSO        | **-**              |                                                                  |
...
- `data_fine` deve essere `NULL` se `stato <> CONCLUSO`. Il valore può essere inserito soltanto durante le transizioni terminali. Deve valere che: `data_fine >= data_creazione` e, se `data_inizio <> NULL`, `data_fine >= data_inizio`.
...
TODO: aggiungere il vincolo che si puo avere solo un progetto in corso (PIANIFICATO o ATTIVO) per lotto.
...
**Coerenza di Lettura**
Data un'istanza di relazione **riceve**, deve valere che: - Se `is_letta` è **FALSE**, allora `data_presa_visione` deve essere **NULL**. - Se `is_letta` è **TRUE**, allora `data_presa_visione` deve essere **NOT NULL**.
